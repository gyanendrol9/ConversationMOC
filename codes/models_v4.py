...
def train_lstm_singletask(epochs, batch_size, x_train, y_train, title, kfold, model_path):
    ...
    optimizer = Adam(lr=0.0001)
    model.compile(optimizer=optimizer, loss=lambda y_true, y_pred: masked_categorical_crossentropy(y_true, y_pred), metrics=['accuracy'])

    # Print the model summary
    model.summary()

    # Define the checkpoint filepath
    filepath = model_path+f"/Fold-{kfold}-{title}"+"_singletask-all_emb-MLF-{epoch:02d}.hdf5"
    checkpoint_callback = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_weights_only=True, period=10)
    ...
    model_json = model.to_json()
    with open(f"{model_path}/singletask-all_emb-{title}-MLF-{epochs}_fold_{kfold}.json", "w") as json_file:
        json_file.write(model_json)
    model.save_weights(f"{model_path}/singletask-all_emb-{title}-MLF-{epochs}_fold_{kfold}.h5")

    filepath = model_path+f"/Fold-{kfold}_singletask-all_emb-{title}-MLF-{epochs}.pdf"
    plot_training_model_singletask(history, filepath=filepath, savefig=True)
    del model
...
def train_lstm_multitask(epochs, batch_size, x_train, y_train, y2_train, title, kfold, model_path):
    ...
    if num_classes <= 2:
        ...
        loss1 = "categorical_crossentropy" #lambda y_true, y_pred: masked_categorical_crossentropy(y_true, y_pred)
    ...
    if num_classes <= 2:
        ...
        loss1 = lambda y_true, y_pred: masked_categorical_crossentropy(y_true, y_pred)
    ...
    # Define the checkpoint filepath
    filepath = model_path+f"/Fold-{kfold}-{title}"+"_multitask-all_emb-MLF-{epoch:02d}.hdf5"
    checkpoint_callback = ModelCheckpoint(filepath, monitor='val_dense_1_loss', verbose=1, save_weights_only=True, period=10)

    # Define early stopping callback
    early_stop = tf.keras.callbacks.EarlyStopping(monitor='val_dense_1_loss', patience=10)

    if not os.path.exists(f"{model_path}/multitask-all_emb-{title}-MLF-{epochs}_fold_{kfold}.h5"):
        history = model.fit(x_train, [y_train, y2_train], batch_size=batch_size, epochs=epochs, validation_split=0.1, callbacks=[checkpoint_callback, early_stop], verbose=1)
            
        model_json = model.to_json()
        with open(f"{model_path}/multitask-all_emb-{title}-MLF-{epochs}_fold_{kfold}.json", "w") as json_file:
            json_file.write(model_json)
        model.save_weights(f"{model_path}/multitask-all_emb-{title}-MLF-{epochs}_fold_{kfold}.h5")

        filepath = model_path+f"/Fold-{kfold}_multitask-all_emb-{title}-MLF-{epochs}.pdf"
        plot_training_model_multitask(history, filepath=filepath, savefig=True)
